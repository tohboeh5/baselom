...
# Lint all code (Rust + Python) - includes format check
mise run lint
...
mise run lint-rust      # includes cargo fmt --check + clippy
mise run test-rust

# Python only
mise run format-python
mise run lint-python    # ruff check
...
- `mise run lint` - Format check + linting (clippy, ruff)
...
2. **Always lint before committing**: Use `mise run lint` to catch issues early (includes format check)
